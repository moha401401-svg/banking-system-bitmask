...
public:
    ...
    static vector<string> BeforeAfterSeperator(string word, string sep) {
        vector<string>ss(2);
        for (size_t i = 0; i < word.length(); i++)
        {
            if (word[i] == sep[0]) {
                if (i + sep.length() > word.size()) { continue; }
                string ch = word.substr(i, sep.length());
                if (ch == sep) {
                    ss[0] = (word.substr(0, i));
                    ss[1] = word.substr(i + sep.length());
                    return ss;
                }
